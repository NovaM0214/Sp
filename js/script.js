class SpotBonusGenerator {
    ...
    preloadFrameImages() {
        this.frameImages = {};
        const frameFiles = ['headkuang.png', '上框.png', '下框.png', '两侧框.png'];
    ...
    createPersonalFrame(imageData) {
        return new Promise((resolve) => {
            ...
            img.onload = () => {
                ...
                ctx.restore();
                // 绘制前景框架
                if (this.frameImages['headkuang.png']) {
                    ctx.imageSmoothingEnabled = true;
                    ctx.imageSmoothingQuality = 'high';
                    ctx.drawImage(this.frameImages['headkuang.png'], 0, 0, canvas.width, canvas.height);
                }
    ...
    drawTeamFrames(ctx, width, height) {
        ...
        if (this.frameImages['上框.png']) {
            const topFrame = this.frameImages['上框.png'];
            topHeight = topFrame.height * (width / topFrame.width);
        }
        if (this.frameImages['下框.png']) {
            const bottomFrame = this.frameImages['下框.png'];
        ...
        // 1. 绘制上框（顶部对齐）
        if (this.frameImages['上框.png']) {
            ctx.drawImage(this.frameImages['上框.png'], 0, 0, width, topHeight);
        }
        // 2. 绘制两侧框（紧接上框下方，底端与下框顶端衔接）
        if (this.frameImages['两侧框.png'] && sideHeight > 0) {
            ctx.drawImage(this.frameImages['两侧框.png'], 0, topHeight, width, sideHeight);
        }
        // 3. 绘制下框（底部对齐）
        if (this.frameImages['下框.png']) {
            ctx.drawImage(this.frameImages['下框.png'], 0, height - bottomHeight, width, bottomHeight);
        }
    }
